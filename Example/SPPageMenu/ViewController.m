...
- (void)viewDidLoad {
    ...
    self.titles = @[@"跟踪器样式专区",@"排列方式专区",@"其余功能专区",@"特别属性"];
    self.dataSource = @[@[@"下划线与按钮等宽(默认)",@"下划线比按钮略长",@"下划线“依恋”样式",@"缩放",@"圆角矩形",@"圆角矩形（与pageMenu同时圆角）",@"矩形"],@[@"可滑动的自适应内容排列",@"不可滑动的等宽排列",@"不可滑动的自适应内容排列"],@[@"显示功能按钮(显示在最右侧)",@"给功能按钮设置图片和文字", @"含有图片的按钮",@"指定按钮携带图片,或同时携带图片和文字,可以设置图片的位置",@"关闭跟踪器的随时跟踪效果"],@[@"bridgeScrollView属性"]];
}
