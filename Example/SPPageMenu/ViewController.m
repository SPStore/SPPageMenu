...
- (void)viewDidLoad {
    ...
    self.dataSource = @[@[@"下划线与item等宽(默认)",@"下划线比item略长",@"下划线“依恋”样式",@"文字颜色渐变且缩放",@"圆角矩形",@"矩形"],@[@"可滑动的自适应内容排列",@"不可滑动的等宽排列",@"不可滑动的自适应内容排列"],@[@"显示功能按钮(显示在最右侧)",@"给功能按钮设置图片和文字", @"含有图片的item",@"指定item携带图片,或同时携带图片和文字,可以设置图片的位置",@"关闭跟踪器的随时跟踪效果"],@[@"简单属性和简单方法"],@[@"bridgeScrollView属性"]];
}
